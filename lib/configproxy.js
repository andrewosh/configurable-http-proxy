...
var url = 'mongodb://localhost:27017/routes';
...
var ConfigurableProxy = function (options) {
    ...
    this.url = 'mongodb://localhost:27017/routes';
    this.db = new db.Manager(this.url);
    ...
    // populate the trie with all routes stored in the DB
    this.db.get_all_routes(function(routes) {
        routes.forEach(function(route) {
            that.add_route(route["path"], route["data"])
        });
    });
};
...
ConfigurableProxy.prototype.post_routes = function (req, res, path, data) {
    ...
    // Write the route after the response has been sent
    this.db.write_route(path, data)
};
...
ConfigurableProxy.prototype.delete_routes = function (req, res, path) {
    ...
    // Delete the route after the response has been sent
    this.db.delete_route(path)
};
...
ConfigurableProxy.prototype.handle_proxy = function (kind, req, res) {
    ...
    args.push(function (e) {
        ...
        if (res.write) res.write("<script>setTimeout(function() {location.reload();}, 500);</script>");
        if (res.end) res.end();
    });
